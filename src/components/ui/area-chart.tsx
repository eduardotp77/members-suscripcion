...
export const chartCssVars = {
  ...
  foregroundMuted: "hsl(var(--chart-foreground-muted))",
  label: "hsl(var(--chart-label))",
  linePrimary: "hsl(var(--chart-line-primary))",
  lineSecondary: "hsl(var(--chart-line-secondary))",
  crosshair: "hsl(var(--chart-crosshair))",
  grid: "hsl(var(--chart-grid))",
  indicatorColor: "hsl(var(--chart-indicator-color))",
  indicatorSecondaryColor: "hsl(var(--chart-indicator-secondary-color))",
  markerBackground: "hsl(var(--chart-marker-background))",
  markerBorder: "hsl(var(--chart-marker-border))",
  markerForeground: "hsl(var(--chart-marker-foreground))",
  badgeBackground: "hsl(var(--chart-marker-badge-background))",
  badgeForeground: "hsl(var(--chart-marker-badge-foreground))",
  segmentBackground: "hsl(var(--chart-segment-background))",
  segmentLine: "hsl(var(--chart-segment-line))",
};
...
function DateTicker({ currentIndex, labels, visible }: DateTickerProps) {
  ...
  const monthIndices = useMemo(() => {
    ...
    parsedLabels.forEach((label, index) => {
      ...
      if (uniqueMonths.length === 0 || lastMonth !== label.month) {
...
export function ChartTooltip({
  ...
  const title = useMemo(() => {
    ...
    return xAccessor(tooltipData.point).toLocaleDateString("es-ES", {
      weekday: "short",
      month: "short",
      day: "numeric",
    });
...
export function XAxis({ numTicks = 5, tickerHalfWidth = 50 }: XAxisProps) {
  const { xScale, margin, tooltipData, containerRef, data, xAccessor } = useChart();
  const [mounted, setMounted] = useState(false);
  ...
  const labelsToShow = useMemo(() => {
    ...
    // Extraer todos los meses únicos de los datos reales
    ...
    data.forEach((d) => {
      ...
      if (!uniqueMonths.has(monthKey)) {
        uniqueMonths.set(monthKey, date);
      }
    });

    // Convertir a array y ordenar por fecha
    const monthEntries = Array.from(uniqueMonths.entries())
      .sort((a, b) => a[1].getTime() - b[1].getTime());

    // Generar labels con posiciones X correctas
    return monthEntries.map(([label, date]) => ({
      date,
      x: (xScale(date) ?? 0) + margin.left,
      label,
    }));
  }, [xScale, margin.left, data, xAccessor]);
...
function ChartInner({
  ...
  const dateLabels = useMemo(
    () =>
      data.map((d) =>
        xAccessor(d).toLocaleDateString("es-ES", {
          month: "short",
          day: "numeric",
        })
      ),
